export   const messages = [
    {
      id: 1,
      title: "About the day i saw a cutie🌟",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741631336/memories/aqpbcyldnnhkn2mhglut.jpg",
      message: "Ok so this is about when I first saw you wese to uk this pr i found you thora more girly and merko laga tha ki i should make you my bestie😂❤🫂 our vibes may match 🤭💖 but but cutt to today...and yes you are  my bestiee😭🫂❤ you are my love 😘 you are my everything 😭💖 I LOVEE YOU MY PRETTY LITTLE BABE🤭😂💖"
    },
    {
      id: 2,
      title: "Whats BESSTESTT in You",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1742047250/memories/peinsgsptataftfolsj5.jpg",
      message: "You are best in evrything but  for this cutie msg 🥹 ik its difficult to write these msgs in these orders but I just want to let you know that I lovee every thing about youu specially the cute height difference between us😭❤ those hugs those hand holds and you resting your head on my shoulders in my metro is the best part of my metro life 😭💖❤ tera. bina ya khanda kisi kam ka ni lgta 😕🫂..Teri har chiz se muje pyaar h teri smile teri smell tere cute jokes tere pyar bhare snaps jisme tu sundar se filter lagake merko hasati h😂🫂😭 snaps itni to bheji ni apne jitne mene restore kri h"
    },
    {
      id: 3,
      title: "Best Memories",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741604167/memories/daoeh3bqqdnvap8autir.jpg",
      message: "These best memory I could remember of us is when we first met at nsp mall on our first solo date🥹❤🫂 the way u looked at me and ik i m gone for my life😂🫂❤ then wo 6 months pe when we met and u cried watching the video😭😭took my heart away from me🥹😂 terko ese sweet sweet gifts deke rulane ka karyakaram hamesha chlta rahege kabhi bnd ni hoga,💖❤🫂 there are millions of memories i could remember of you every moment and everytime I just smile and get scared ki what if I lose you😔🫂but in every universe I will still lovee you as much as I do today😭😭💖❤❤"
    },
    {
      id: 4,
      title: "Apki Thori Aur TAREEF",
      image:"https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741594099/memories/soqw8rgftvibmqprknsy.jpg",
      message: "Apart from memories i admire you alot me terse bahut kuch seekhta hu🫂❤ merko sehne jitna patience rkhna😂 sbki baski bt ni only you can handle a cutie like me🥹💗 i always look out for best things for us and specially you because ik you r my pretty little baby and I have to take care of you❤💗🫂🫂 wrna godi lo godi lo krne lgegi🤣🤣😭🫂💖 ek br godi leke pani me fek chuka hu ab br br mt bolna wrna agli br ese jagah fekuga yd rahega hmesha🤭💗"
    },
    {
      id: 5,
      title: "Few shabad on Your SENSE OF HUMOUR",
      image:"https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741551375/memories/gxze5iopysi1a4nvniet.jpg",
      message: "Haha yehi sochri na kesa sa khrbh sense of humor hai iska ..lemme remind you that😶 ye sense of humor apke ane se aisa hua ise pehle me bahut ache joke marta th🥹🤭 mere dosto se ouchlena sb bta dege🥰 and apke jokes to aise hote hai ki me roo du"
    },
    // Add 15 more messages...
    {
      id: 6,
      title: "Us For EACH OTHER",
       image:"https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741594099/memories/soqw8rgftvibmqprknsy.jpg",
      message:"Prrr rone se yd aaya kuch merko🥹❤ you are the one and only one jiske age me roo skta hu😔🫂 jitna b sad hou ik i can express it with you and you will be there to console me 🫂❤  you are my biggest supporter you celebrate my success like yours (party mangle🤣🤣) and I also love the way u r with me in my tougher times when I have no one I have you with me😔🫂😭 you are the best one I could get in this jalim dunia ..teri tareef b kese kru tareef shabad apke liye kam pdega😭😭"
    },
    {
      id: 7,
      title: "What good hAve you donee",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741547297/memories/nvrbnasx5gxnk3mm9hth.jpg",
      message: "Watching you chase your dreams with such determination inspires everyone around you. Never stop believing in yourself!"
    },
    {
      id: 8,
      title: "Teri Pyari Harqate",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741545464/memories/htc3dme8uezp5wkoqtfw.jpg",
      message: "Your adventurous spirit and love for exploring new places always amazes us. The world is your playground!"
    },
    {
      id: 9,
      title: "Some greatest Memories",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741510586/memories/lj2bwqrgqm25cxati6oi.jpg",
      message: "On top its jhula pool parites"
    },
    {
      id: 10,
      title: "You are my real motivation",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741509699/memories/x17qtpyn2grblopmju1p.jpg",
      message: "Every birthday has been special, but this 20th one is extra magical! You're stepping into an exciting new decade!"
    },
    {
      id: 11,
      title: "Pretty Little Baby Vibes",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741507915/memories/mwicgy58xy0atkao0fqi.jpg",
      message: "Despite being young, you have wisdom beyond your years. Your advice and perspective always amazes everyone!"
    },
    {
      id: 12,
      title: "Future Niyati",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample2.jpg",
      message: "Your dreams are valid and achievable! Keep reaching for the stars because you deserve all the success in the world!"
    },
    {
      id: 13,
      title: "You & just You",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1755028111/WhatsApp_Image_2025-08-12_at_19.46.04_94ce6501_vyp0kl.jpg",
      message: "Your infectious laughter can light up any room! Thank you for bringing so much joy into our lives!"
    },
    {
      id: 14,
      title: "A lil bit about the unsaid side of youuu",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1755028101/WhatsApp_Image_2025-08-12_at_19.46.04_2165251e_qlbo4z.jpg",
      message: "You're always there to lend a helping hand or a listening ear. Your support means the world to everyone who knows you!"
    },
    {
      id: 15,
      title: "Our BOND",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741500540/memories/z7epjedtf7gxqqil7npb.jpg",
      message: "There's absolutely no one like you in this world! Your unique personality and quirks make you absolutely perfect!"
    },
    {
      id: 16,
      title: "Us together",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1755028044/WhatsApp_Image_2025-08-12_at_19.44.51_d5ff386d_yiaomq.jpg",
      message: "Watching you grow and evolve over the years has been incredible. You've become such a wonderful person!"
    },
    {
      id: 17,
      title: "Hamare secrets laugh emoji",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1755028251/WhatsApp_Image_2025-08-13_at_01.20.25_622dc404_ee3x8l.jpg",
      message: "Your talents and abilities continue to amaze us! Whether it's singing, dancing, or any other skill - you excel at everything!"
    },
    {
      id: 18,
      title: "What i Wish For You in my prayers",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1741630955/memories/vqenwpdbrz9aii04ziak.jpg",
      message: "So many beautiful memories have been created with you! Each moment spent together is a treasure we'll always cherish!"
    },
    {
      id: 19,
      title: "What changed after you came",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1755028241/WhatsApp_Image_2025-08-13_at_01.20.26_cd7f96e2_jwvqvj.jpg",
      message: "The future holds so many amazing opportunities for you! We can't wait to see all the incredible things you'll achieve!"
    },
    {
      id: 20,
      title: "Finale Msg",
      image: "https://res.cloudinary.com/dhe9p6bo0/image/upload/v1755028246/WhatsApp_Image_2025-08-13_at_01.20.26_ea586659_ebghvx.jpg",
      message: "We're forever grateful to have you in our lives! Happy 20th Birthday! May this year bring you endless happiness, love, and success! 🎉❤️"
    }
  ] 
  
  export const gifts = [
    {
      id: 1,
      title: "This Website",
      description: "As an Web Dev Its the first gift",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample.jpg",
      message: "THis website is literally made in less than 4 hours i just made it using ai but the whole idea is mine the execution the connections the pics the msgs are mine so hehe count this in ",
      color: "from-pink-400 to-rose-500"
    },
    {
      id: 2,
      title: "Necklace",
      description: "A symbol of our unbreakable love",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample2.jpg",
      message: "This special necklace is my choice its literally real silver and i am so nervouse terko psnd bhi ayega ya ni because i havent brought this much jwellery in past so kesa laga btana jrur hehe",
      color: "from-purple-400 to-indigo-500"
    },
    {
      id: 3,
      title: "Its a Keychain",
      description: "Guess the design cutiee",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample3.jpg",
      message: "Sometime back i got to know about that you love Some boy called HARRY POTTER kon h ye ladkaa ..khair ab mene apke liye ek keychain la di taki you dont miss him yd rkhuga hmesha kisne cheena apko merse",
      color: "from-green-400 to-emerald-500"
    },
    {
      id: 4,
      title: "The Obvious One",
      description: "See The Design ",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample4.jpg",
      message: "Meri nadani se ye gift bahut pehle reveal hogya pr i m sure u would love the deisgn mene 10 dukane dhundi h aise kuch ache chiz k liye so please tell me you love it only nothing elsee",
      color: "from-blue-400 to-cyan-500"
    },
    {
      id: 5,
      title: "Gift Box",
      description: "The artist inside me made something",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample5.jpg",
      message: "A beautiful box that has alot of things u like along with your gifts too hehe to much mehnat",
      color: "from-yellow-400 to-orange-500"
    },
    {
      id: 6,
      title: "Spiced Cappucino Time",
      description: "Chal vlog banate haiii",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample.jpg",
      message: "! A little meaningfull gift and its for the youtuber inside you i m always with u in this journey and will always support you till the extend i can and here is a small gesture from my side",
      color: "from-red-400 to-pink-500"
    },
    {
      id: 7,
      title: "Spiced Cappucino Time",
      description: "Chal vlog banate haiii",
      image: "https://res.cloudinary.com/demo/image/upload/w_300,h_200,c_fill/sample.jpg",
      message: "! A little meaningfull gift and its for the youtuber inside you i m always with u in this journey and will always support you till the extend i can and here is a small gesture from my side",
      color: "from-red-400 to-pink-500"
    }
  ]

  export const recentPhotos = [
    {
      id: 1,
      url: "https://res.cloudinary.com/demo/image/upload/w_300,h_300,c_fill/sample.jpg",
      caption: "That amazing day at the beach! 🏖️"
    },
    {
      id: 2,
      url: "https://res.cloudinary.com/demo/image/upload/w_300,h_300,c_fill/sample2.jpg",
      caption: "Coffee dates are the best dates ☕"
    },
    {
      id: 3,
      url: "https://res.cloudinary.com/demo/image/upload/w_300,h_300,c_fill/sample3.jpg",
      caption: "Adventures in the city 🏙️"
    },
    {
      id: 4,
      url: "https://res.cloudinary.com/demo/image/upload/w_300,h_300,c_fill/sample4.jpg",
      caption: "Movie night was epic! 🍿"
    }
  ]

  export const notes = [
    "You make every ordinary day feel extraordinary! ✨",
    "Your smile is contagious and lights up everyone's day 😊",
    "Thank you for being such an amazing person in our lives 💕",
    "Here's to more adventures and memories together! 🎉",
    "Your 20s are going to be absolutely incredible! 🚀"
  ]